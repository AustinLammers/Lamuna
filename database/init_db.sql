...
CREATE TABLE ExerciseLogs (
    ...
    calories_burnt INT,
    user_id INT NOT NULL,
    CONSTRAINT fk_exercise_user FOREIGN KEY (user_id) REFERENCES Users (user_id)
);

-- Indexes for FK lookups
CREATE INDEX idx_foodlogs_user ON FoodLogs(user_id);
CREATE INDEX idx_exerciselogs_user ON ExerciseLogs(user_id);
...
INSERT INTO ExerciseLogs (name, description, workout_type, time, calories_burnt, user_id)
